...
function getAllTokens(): Token[] {
  const tokensData = JSON.parse(fs.readFileSync('binance_tokens.json', 'utf8'));
...
function updateScrapingQueue(): Token[] {
  ...
  const currentFileHash = getFileHash('binance_tokens.json');
...
function displayQueueStatus(): void {
  ...
  try {
    ...
    console.log('Could not load binance_tokens.json for comparison');
  }
  console.log('=====================================\n');
}
...
function addTokenToQueue(symbol: string): boolean {
  try {
    ...
    if (!tokenToAdd) {
      console.log(`Token ${symbol} not found in binance_tokens.json`);
      return false;
    }
...
(async () => {
  ...
  if (fs.existsSync('binance_tokens.json')) {
    fs.watchFile('binance_tokens.json', { interval: 5000 }, async (curr, prev) => {
      const tokens = getAllTokens();
